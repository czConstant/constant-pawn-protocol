...
import {
  ...
  getPublicKey,
  getTerms,
  getTokenBalance, InitOrderInstruction, logError, OfferInfoLayout,
...
const order_now = async () => {
  ...
  const transferUsdToTempAccIx = Token.createTransferInstruction(
    ...
    lender_account.publicKey,
    [],
    Number(terms.loanPrincipalAmount),
  );
  ...
  const initOrderIx = InitOrderInstruction(
    lendingProgramId,
    lender_account.publicKey,
  ...
  console.log("Sending order transaction...");
